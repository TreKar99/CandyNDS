...
void genera_mapa1(char mat[][COLUMNS])
{
    ...
    for(i = 0; i<ROWS; i++) {    //para todas las filas
        for(j = 0; j<COLUMNS; j++) {    //para todas las columnas
            if(mat[i][j] < 7 || mat[i][j] == 15)     //donde no exista bloque sólido ni gelatina, 
                fija_metabaldosa((void *)0x06000000, i, j, 19);    //copiar una metabaldosa transparente (índice 19)
            else if(mat[i][j] == 7)         //donde exista un bloque sólido, 
                fija_metabaldosa((void *)0x06000000, i, j, 16);    //copiar la metabaldosa que representa una verja (índice 16)
				...
				} else {													//metabaldosa de animación de gelatina (simple o doble)
					rand = rand + 8;
					fija_metabaldosa((void *)0x06000000, i, j, rand);
				}
				mat_gel[i][j].im = rand;
				rand = mod_random(10)+1;
				mat_gel[i][j].ii = rand;		//setteamos índice de animación random del 1 al 10
			}
...
// Tareas 2Ba (patrón ajetreado) y 2Ca:
